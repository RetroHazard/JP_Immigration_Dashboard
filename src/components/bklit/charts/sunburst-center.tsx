...
export const SunburstCenter = memo(function SunburstCenter({
  ...
  const { arcs, focus, prevFocus, maxDepth, radius, zoomT, zoomTo, getColor } =
    useSunburstStable();
  ...
  // Local extension: honor the focused node's own color override (as segments
  // do) instead of always falling back to the default palette.
  const focusArcColor = arcs.find((arc) => arc.id === focus.id)?.color;
  const centerColor =
    focus.depth === 0
      ? sunburstCssVars.background
      : getColor(focus.categoryIndex, focusArcColor);
